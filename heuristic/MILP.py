...
#######    DATA LOAD    ###############
file = open("/Users/xinshi/Documents/research-powersystem2/testimpl/implementation1/more57/0.1/ieee57_scenarios.dat","r")
...
for i in m.getVars():
	...
	if i.varName.split('[')[0] == 'location':
		if int(i.x) == 1:
			print('%s %g' % (i.varName, i.x))
	if i.varName.split('[')[0] == 'node_to_microgrid':
         if int(i.varName.split('[')[1].split(',')[-1].split(']')[0]) == 0:
            if int(i.x) == 1:
                print('%s %g' % (i.varName, i.x))
